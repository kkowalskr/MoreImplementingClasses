...
class Line(object):
    ...
    def length(self):
        ...
        return ((self.start.x - self.end.x) ** 2 + (self.start.y -
                                                   self.end.y) ** 2) ** 0.5
    ...
    def line_plus(self, other_line):
        ...
        start_point = Point(self.start.x + other_line.start.x, self.start.y
                            +other_line.start.y)
        end_point = Point(self.end.x + other_line.end.x, self.end.y
                          + other_line.end.y)
    ...
    def is_parallel(self, line2):
        ...
        if round(self.slope(), 9) == round(line2.slope(),9):
            return True
        else:
            return False
